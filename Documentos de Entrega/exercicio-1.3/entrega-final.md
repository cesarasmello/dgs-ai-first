# Entrega Final - Exercicio 1.3


### Versao inicial da especificacao:

- **Documento de entrada:** `anexo-a-documentacao-simulada-novatech.md`, `anexo-b-chunks-referencia-rag.md` e `jornada-atendente-assistente-ia.md`. 
...
Seu objetivo é produzir uma Especificação de Requisitos do Produto para um assistente de atendimento com RAG, em linguagem de negócio (não técnica), orientada à operação real do atendimento.
...
- Considere o documento 'anexo-a-documentacao-simulada-novatech.md` como exemplo concreto das contradições e gaps que a spec precisa endereçar e demais documentos operacionais anexados.

- Considere o seguinte conceito de pipeline de RAG: "Documentos são divididos em pedaços (chunks), transformados em representações numéricas (embeddings), armazenados num banco vetorial, e recuperados por similaridade quando o usuário faz uma pergunta. O LLM então gera uma resposta usando os chunks recuperados como contexto."
...
- Regras obrigatórias:
    ...
    - Destacar explicitamente riscos de curadoria de conteúdo (obsolescência, duplicidade, conflito de versão, ausência de dono).
...
- Formato obrigatório de saída: 
    ...
    3. Requisitos funcionais (tabela):
        - ID
        - Requisito
        - Justificativa de negócio
        - Critério de aceitação (testável)
        - Evidência documental esperada

    4. Requisitos não funcionais (tabela):
        -ID
        -Requisito
        -Métrica/SLA
        -Como validar
...
```
- **Output:** Arquivo `PRD-NovaTech-Assistente-RAG-v1` - Especificação de Requisitos do Produto - Versão 1

---

### Feedback obtido na iteracao:

- **Documento de entrada:** `PRD-NovaTech-Assistente-RAG-v1`
...
- **Script:**
    ...
    Analise o documento de Especificação de Requisitos do Produto (PRD) anexado e identifique gaps, ambiguidades e inconsistências.

    - Regras da análise:
        - Avalie clareza, testabilidade, completude, consistência, rastreabilidade e viabilidade operacional.
        - Considere ambiguidades de linguagem, ausência de critérios de aceite, conflitos entre seções, requisitos não mensuráveis e lacunas de decisão.
...
- **Documento de entrada:** `PRD-NovaTech-Assistente-RAG-v1` e `Feedback.md`. 
...
```
    ...
    - Tarefa: 
        - Refinar o documento de especificação (PRD) com base no feedback recebido, preservando o escopo original e aumentando clareza, testabilidade e consistência. 
        - Aplique apenas mudanças justificadas pelo feedback, sem conhecimento externo, mantendo o escopo original.
...
- **Output:** Arquivo `PRD-NovaTech-Assistente-RAG-v2` - Especificação de Requisitos do Produto - Versão 2