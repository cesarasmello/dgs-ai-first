...
|---|---|---|---|
| BC1 — Devolução de Mercadorias | 14 | Prazo de devolução, Elegibilidade, Coleta reversa, Triagem, Reembolso, Frete reverso, CT-e, Devolução parcial | 1 (data-base quando tracking falha) |
| BC2 — Precificação de Frete Especial | 10 | Frete especial, Valor base, Multiplicador regional, Fator de peso, Desconto por volume, Prazo adicional | 2 (versão vigente PROC-042, frete padrão) |
| BC3 — Gestão de SLA e Atendimento | 10 | Tier de cliente, SLA de resposta, SLA de resolução, Incidente crítico, Penalidade por violação, Relógio de SLA | 0 |
| BC4 — Conformidade e Riscos de Carga | 7 | Carga perigosa, Cadeia de frio, Lacre de segurança, Gestão de Riscos, Classe ANTT | 1 (processo pós-ramal 4500) |
| BC5 — Sinistros e Carga Danificada | 5 | Carga danificada, Registro de ocorrência, Sinistro, Responsabilidade da NovaTech | 1 (ausência de POL/PROC formal) |
| BC6 — Seguro de Carga | 3 | Seguro de carga, Percentual de seguro | 1 (ausência de documento normativo) |
| **Total** | **49** | **—** | **6** |
...
| **Regra de desambiguação para LLM** | Se o cliente mencionar carga danificada *durante o transporte*, não tratar como devolução — redirecionar para BC5 (Sinistros). Se mencionar carga perigosa, não processar como devolução padrão — redirecionar para BC4. |
...
#### Termo 4: Triagem de chamado de devolução

| Campo | Conteúdo |
|---|---|
| **Termo canônico** | Triagem de chamado de devolução |
| **Definição oficial** | Verificação realizada pelo time de atendimento em até 4 horas úteis após a abertura do chamado, que avalia simultaneamente: (a) se a carga é elegível para devolução padrão, (b) se a documentação está completa (CT-e, 3 fotos, motivo), e (c) se o prazo de 7 dias úteis ainda vigora. Resulta em aprovação ou recusa do chamado. |
| **Contexto Dono** | BC1 — Devolução de Mercadorias |
| **Contextos Relacionados** | BC3 (o prazo de 4h úteis é regra interna de BC1; o SLA de BC3 pode exigir resposta antes disso para clientes Gold) |
| **Fora de escopo** | Não inclui a decisão sobre custo da devolução — essa é tomada após aprovação. |
| **Regra de desambiguação para LLM** | O prazo de 4 horas úteis de triagem (BC1) é diferente do SLA de primeira resposta de BC3. Para cliente Gold, o SLA de BC3 exige resposta em até 2h úteis — que pode ser anterior à conclusão da triagem. Resposta ≠ conclusão de triagem. |
| **Exemplo válido** | Chamado aberto às 9h (segunda). Triagem concluída até 13h do mesmo dia. |
| **Confusão comum a evitar** | "Triagem aprovada" não significa que a coleta foi agendada — coleta é etapa posterior (até 2 dias úteis após aprovação). |
...
| **Regra de desambiguação para LLM** | Usar +3 dias úteis (v2) para chamados novos. Se cliente citar +2 dias, informar que a versão vigente é v2. |
| **Exemplo válido** | Rota com prazo padrão de 5 dias úteis + frete especial = 8 dias úteis de prazo total (v2). |
| **Confusão comum a evitar** | O prazo adicional é somado ao prazo padrão da rota, que não está documentado no escopo disponível. O assistente pode informar o adicional (+3d) mas não pode calcular o prazo total sem o prazo base da rota. |
...
| **Confusão comum a evitar** | O desconto incide sobre o *multiplicador regional*, não sobre o valor total do frete. A v1 exigia mais de 10 fretes/mês e não definia percentuais — usar apenas v2. |
...
| **Contexto Dono** | BC3 — Gestão de SLA e Atendimento |
| **Contextos Relacionados** | BC1 (triagem de 4h pode conflitar com SLA de resposta de 2h para Gold — resposta deve ocorrer antes da conclusão da triagem, se necessário) |
...
| **Confusão comum a evitar** | "Alta prioridade" mencionada pelo cliente não equivale a incidente crítico — a classificação é objetiva, baseada nos 4 critérios acima, não na percepção do cliente. |
...
| **Exemplo válido** | 3ª violação de SLA em março para cliente Silver → crédito de 10% sobre o frete do chamado + reunião com gerente de operações. |
...
| **Definição oficial** | Contador de tempo que mede o cumprimento do SLA a partir do timestamp de abertura do chamado no Azure DevOps. Para chamados gerais: pausa fora do horário comercial (18h às 8h, dias úteis, exceto feriados). Para incidentes críticos de clientes Gold: não pausa em nenhuma hipótese. |
| **Contexto Dono** | BC3 — Gestão de SLA e Atendimento |
| **Contextos Relacionados** | — |
| **Fora de escopo** | Não se aplica ao prazo de devolução (BC1) nem ao prazo de entrega (BC2). |
| **Regra de desambiguação para LLM** | Para incidentes críticos Gold, calcular o tempo sempre em horas corridas. Para chamados gerais, descontar o período fora do horário comercial. |
| **Exemplo válido** | Chamado geral Gold aberto às 17h30 → SLA pausa às 18h, retoma às 8h do dia seguinte. Dos 30 min da tarde, usados 30 min do SLA de 2h. |
| **Confusão comum a evitar** | Incidentes críticos Gold não pausam — um chamado crítico Gold aberto às 23h tem SLA de 30 min correndo até 23h30. |
...
| **Regra de desambiguação para LLM** | Ao identificar carga perigosa em qualquer cenário (devolução, frete, seguro), acionar BC4. Não processar como carga padrão. A presença de carga perigosa altera o fluxo em *todos* os contextos relacionados. |
| **Exemplo válido** | Cliente solicita devolução de substância inflamável (classe 3) → não elegível pelo processo padrão → encaminhar ao ramal 4500. |
| **Confusão comum a evitar** | "Carga perigosa" tem definição técnica específica (classes ANTT). Não classificar como perigosa com base em percepção subjetiva do cliente. |
...
| **Regra de desambiguação para LLM** | O critério de ruptura é objetivo: temperatura fora da faixa *por mais de 30 minutos contínuos* conforme sensor IoT. Sem registro do sensor, não é possível confirmar ou negar ruptura — encaminhar à Gestão de Riscos. |
| **Exemplo válido** | Carne bovina com faixa de -18°C a -15°C. Sensor registra -10°C por 45 min contínuos → cadeia rompida → não elegível para devolução padrão. |
| **Confusão comum a evitar** | "Carga refrigerada" por si só não impede devolução — somente se a cadeia de frio estiver *rompida*. |
...
| **Regra de desambiguação para LLM** | Gestão de Riscos ≠ Jurídico (BC5) ≠ Compliance. São setores diferentes com escopos distintos. Gestão de Riscos atua em BC4; Jurídico atua em BC5; Compliance atua na autorização de cargas perigosas para frete expresso (não formalizado). |
| **Exemplo válido** | Cliente tenta devolver carga de gás (classe 2 ANTT) → assistente encaminha para ramal 4500 (Gestão de Riscos). |
...
| **Definição oficial** | Mercadoria que apresenta dano físico ocorrido durante o transporte pela NovaTech, identificado pelo cliente após o recebimento. Segue processo distinto da devolução padrão: registro de ocorrência em até 48 horas, com fotos e laudo, encaminhado a sinistros@novatech.com.br para apuração pelo Jurídico. |
...
| **Definição oficial** | Notificação formal do cliente sobre dano à carga durante o transporte, que deve ser feita em até 48 horas após o recebimento, acompanhada de fotos e laudo (quando disponível), enviada ao e-mail sinistros@novatech.com.br. |
| **Contexto Dono** | BC5 — Sinistros e Carga Danificada |
| **Contextos Relacionados** | — |
| **Fora de escopo** | O prazo de 48h de BC5 é distinto do prazo de 7 dias úteis de BC1. Não confundir os dois. |
| **Regra de desambiguação para LLM** | Para sinistros, o prazo é de 48 horas (corridas, não úteis — definição implícita no FAQ). Para devolução, o prazo é de 7 dias úteis. Aplicar o prazo correto conforme o contexto. |
| **Exemplo válido** | Entrega recebida às 14h de segunda → registro de sinistro deve ser feito até às 14h de quarta. |
| **Confusão comum a evitar** | O prazo de 48h para sinistro não é o mesmo que o prazo de triagem de 4h úteis de BC1. São contextos e prazos completamente independentes. |
...
| Frete expresso para carga perigosa | BC4 — Conformidade | FAQ-32 descreve o processo informalmente, mas não existe PROC formal. PROC-043 (para frete de perigosas) está em revisão pelo Compliance. | Informar ao cliente que frete expresso para carga perigosa pode ser possível com autorização do Compliance, que costuma levar ~2 dias; sem garantia de prazo. Nunca confirmar como processo padrão. | **Alto**: LLM pode confirmar possibilidade ou prazo de autorização com falsa precisão, gerando comprometimento indevido da NovaTech. |
...
- BC1 — Devolução: 10 termos mapeados, cobrindo prazo, elegibilidade, triagem, coleta, custo, reembolso, devolução parcial, CT-e e prazo expirado. ✅
- BC2 — Precificação: 7 termos mapeados, cobrindo frete especial, valor base, multiplicador regional, fator de peso, prazo adicional, desconto por volume e aprovação acima de 5.000 kg. ✅
- BC3 — SLA: 6 termos mapeados, cobrindo tier, SLA de resposta, SLA de resolução, incidente crítico, penalidade e relógio. ✅
- BC4 — Conformidade: 4 termos mapeados, cobrindo carga perigosa, cadeia de frio, lacre e Gestão de Riscos. ✅
- BC5 — Sinistros: 3 termos mapeados, cobrindo carga danificada, registro de ocorrência e ressarcimento. ✅
- BC6 — Seguro: 1 termo mapeado, cobrindo seguro de carga. ✅

**Sem definição circular**
- Todos os termos são definidos por condições objetivas e mensuráveis (prazos em dias/horas, percentuais, limiares de peso/valor). ✅

**Sem linguagem vaga sem condição objetiva**
- Termos como "tratamento especial" aparecem somente como descrição de encaminhamento, não como definição de resultado. ✅

**Sem duplicidade semântica entre contextos**
- "Reembolso" tem três instâncias semanticamente distintas, todas explicitadas: BC1 (crédito de devolução), BC3 (crédito por SLA), BC5 (ressarcimento por sinistro). ✅
- "Prazo" tem três instâncias distintas: BC1 (prazo de solicitação), BC2 (prazo de entrega), BC3 (prazo de atendimento). ✅
- "Carga perigosa" tem dono único (BC4) com delegação explícita para os outros contextos. ✅

**Regras de desambiguação para LLM presentes em todos os termos críticos**
- Todos os 31 termos possuem campo "Regra de desambiguação para LLM" preenchido. ✅

**Lacunas identificadas e rotuladas**
- 6 lacunas registradas na Seção 3, todas com hipótese e nível de risco para o LLM. ✅

**Termos sem documento normativo rotulados como Lacuna**
- BC5 (carga danificada, registro de ocorrência, ressarcimento): todos marcados como `Definido ⚠️` com referência à Lacuna L2. ✅
- BC6 (seguro de carga): marcado como `Definido ⚠️` com referência à Lacuna L3. ✅

**Conflito PROC-042 v1/v2 explicitado em todos os termos afetados**
- Termos 11, 13, 14, 15 e 16 todos contêm aviso de conflito e instrução de uso da v2 como padrão. ✅
