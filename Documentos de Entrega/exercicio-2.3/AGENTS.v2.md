...
- DEVE sinalizar explicitamente respostas de baixa confiança (`low_confidence: true`) e escalar ao supervisor humano quando o confidence score ficar abaixo do limiar definido no SDD do query endpoint (`specs/query-endpoint/requirements.md`) — o valor numérico do limiar é normatizado nesse documento, não nesta seção.
...
- Respostas com confidence score abaixo do limiar normatizado no SDD do query endpoint (`specs/query-endpoint/requirements.md`) DEVEM retornar `low_confidence: true` no payload e acionar o caminho de escalonamento ao supervisor em `response-builder.ts` — nunca suprimir a flag. O valor do limiar não é hardcoded nesta seção; lê-se do SDD.
...
- [`specs/query-endpoint/requirements.md`](specs/query-endpoint/requirements.md) — **SDD / Documento de Spec (requirements v2.0), antecedente normativo:** define o limiar numérico de confidence score, `source_document` obrigatório, detecção de conflito documental e comportamento multi-domínio; é a fonte primária dos guardrails do endpoint de consulta e prevalece sobre esta seção para parâmetros operacionais do endpoint.
- **PRD v2.0 — NovaTech RAG Assistant (antecedente normativo):** define os requisitos de produto, os limiares operacionais e a classificação de go-live blocker que originam estas regras de produto; esta seção é a tradução prescritiva do PRD v2.0 para consumo por agentes de IA.
- **Recorte de domínio — Exercício 2.1 (antecedente normativo):** bounded contexts (BC1–BC6) e linguagem ubíqua dos quais os guardrails e o glossário desta seção derivam; consolidado em [`glossario-linguagem-ubiqua-v2.md`](glossario-linguagem-ubiqua-v2.md).
